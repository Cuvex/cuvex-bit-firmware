/*********************************************************************************/
/********** THIS FILE IS GENERATED BY TOUCHGFX DESIGNER, DO NOT MODIFY ***********/
/*********************************************************************************/
#ifndef SCREEN_FLOW_ENCRYPT_PART_2VIEWBASE_HPP
#define SCREEN_FLOW_ENCRYPT_PART_2VIEWBASE_HPP

#include <gui/common/FrontendApplication.hpp>
#include <mvp/View.hpp>
#include <gui/screen_flow_encrypt_part_2_screen/screen_flow_encrypt_part_2Presenter.hpp>
#include <touchgfx/widgets/Box.hpp>
#include <touchgfx/containers/Container.hpp>
#include <touchgfx/containers/buttons/Buttons.hpp>
#include <touchgfx/widgets/TextArea.hpp>
#include <touchgfx/widgets/TextAreaWithWildcard.hpp>
#include <gui/containers/keyboard_encrypt_password1.hpp>
#include <gui/containers/keyboard_encrypt_password2.hpp>
#include <gui/containers/textArea.hpp>
#include <gui/containers/textAreaSelected.hpp>
#include <touchgfx/widgets/ToggleButton.hpp>
#include <touchgfx/widgets/Image.hpp>
#include <gui/containers/keyboard_encrypt_tagAlias.hpp>
#include <gui/containers/close_btn.hpp>

class screen_flow_encrypt_part_2ViewBase : public touchgfx::View<screen_flow_encrypt_part_2Presenter>
{
public:
    screen_flow_encrypt_part_2ViewBase();
    virtual ~screen_flow_encrypt_part_2ViewBase();
    virtual void setupScreen();
    virtual void handleTickEvent();

    /*
     * Virtual Action Handlers
     */
    virtual void tickEventScreen()
    {
        // Override and implement this function in screen_flow_encrypt_part_2
    }
    virtual void hideKeyboardPasswordPressed()
    {
        // Override and implement this function in screen_flow_encrypt_part_2
    }
    virtual void enterKeyboardPasswordPressed()
    {
        // Override and implement this function in screen_flow_encrypt_part_2
    }
    virtual void multisignedYesPressed()
    {
        // Override and implement this function in screen_flow_encrypt_part_2
    }
    virtual void multisignedNoPressed()
    {
        // Override and implement this function in screen_flow_encrypt_part_2
    }
    virtual void passwordSuccessPressed()
    {
        // Override and implement this function in screen_flow_encrypt_part_2
    }
    virtual void eye1Pressed()
    {
        // Override and implement this function in screen_flow_encrypt_part_2
    }
    virtual void eye2Pressed()
    {
        // Override and implement this function in screen_flow_encrypt_part_2
    }
    virtual void show1KeyboardPasswordPressed()
    {
        // Override and implement this function in screen_flow_encrypt_part_2
    }
    virtual void show2KeyboardPasswordPressed()
    {
        // Override and implement this function in screen_flow_encrypt_part_2
    }
    virtual void multisignedPlusPressed()
    {
        // Override and implement this function in screen_flow_encrypt_part_2
    }
    virtual void multisignedMinusPressed()
    {
        // Override and implement this function in screen_flow_encrypt_part_2
    }
    virtual void multisignedSelectPressed()
    {
        // Override and implement this function in screen_flow_encrypt_part_2
    }
    virtual void hideKeyboardAliasPressed()
    {
        // Override and implement this function in screen_flow_encrypt_part_2
    }
    virtual void showKeyboardAliasPressed()
    {
        // Override and implement this function in screen_flow_encrypt_part_2
    }
    virtual void enterKeyboardAliasPressed()
    {
        // Override and implement this function in screen_flow_encrypt_part_2
    }
    virtual void success1Pressed()
    {
        // Override and implement this function in screen_flow_encrypt_part_2
    }
    virtual void success2Pressed()
    {
        // Override and implement this function in screen_flow_encrypt_part_2
    }
    virtual void biometricYesPressed()
    {
        // Override and implement this function in screen_flow_encrypt_part_2
    }
    virtual void biometricNoPressed()
    {
        // Override and implement this function in screen_flow_encrypt_part_2
    }
    virtual void biometricContinuePressed()
    {
        // Override and implement this function in screen_flow_encrypt_part_2
    }
    virtual void biometricBackPressed()
    {
        // Override and implement this function in screen_flow_encrypt_part_2
    }
    virtual void biometricsSuccessPressed()
    {
        // Override and implement this function in screen_flow_encrypt_part_2
    }

protected:
    FrontendApplication& application() {
        return *static_cast<FrontendApplication*>(touchgfx::Application::getInstance());
    }

    /*
     * Member Declarations
     */
    touchgfx::Box __background;
    touchgfx::Box background;
    touchgfx::Container s3_biometricControl;
    touchgfx::Container s3_1_askBiometricControl;
    touchgfx::TextButtonStyle< touchgfx::BoxWithBorderButtonStyle< touchgfx::ClickButtonTrigger >  >  btn_biometric_yes;
    touchgfx::TextButtonStyle< touchgfx::BoxWithBorderButtonStyle< touchgfx::ClickButtonTrigger >  >  btn_biometric_no;
    touchgfx::TextArea text_info_biometric_1;
    touchgfx::Container s3_2_confirmBiometricControl;
    touchgfx::TextButtonStyle< touchgfx::BoxWithBorderButtonStyle< touchgfx::ClickButtonTrigger >  >  btn_biometric_continue;
    touchgfx::TextButtonStyle< touchgfx::BoxWithBorderButtonStyle< touchgfx::ClickButtonTrigger >  >  btn_biometric_back;
    touchgfx::TextArea text_info_biometric_2;
    touchgfx::TextArea text_info_biometric_3;
    touchgfx::Container s4_multiSignature;
    touchgfx::Container s4_1_askMultiSignature;
    touchgfx::TextButtonStyle< touchgfx::BoxWithBorderButtonStyle< touchgfx::ClickButtonTrigger >  >  btn_multi_yes;
    touchgfx::TextButtonStyle< touchgfx::BoxWithBorderButtonStyle< touchgfx::ClickButtonTrigger >  >  btn_multi_no;
    touchgfx::TextArea text_info_multi_1;
    touchgfx::Container s4_2_numMultiSignature;
    touchgfx::TextButtonStyle< touchgfx::BoxWithBorderButtonStyle< touchgfx::ClickButtonTrigger >  >  btn_multi_select;
    touchgfx::IconButtonStyle< touchgfx::ClickButtonTrigger >  btn_multi_plus;
    touchgfx::IconButtonStyle< touchgfx::ClickButtonTrigger >  btn_multi_minus;
    touchgfx::TextAreaWithOneWildcard text_multi_num;
    touchgfx::TextArea text_info_multi_2;
    touchgfx::TextArea text_info_multi_3;
    touchgfx::TextArea text_info_multi_4;
    touchgfx::TextArea text_info_multi_5;
    touchgfx::Container s5_password;
    touchgfx::Container s5_1_typePasword;
    keyboard_encrypt_password1 keyboard1_password;
    keyboard_encrypt_password2 keyboard2_password;
    touchgfx::IconButtonStyle< touchgfx::ClickButtonTrigger >  keyboard_btn_hide_password;
    textArea keyboard1_text_area_password;
    textArea keyboard2_text_area_password;
    textAreaSelected keyboard1_text_area_selected_password;
    textAreaSelected keyboard2_text_area_selected_password;
    touchgfx::TextAreaWithOneWildcard keyboard1_text_typed_password;
    touchgfx::TextAreaWithOneWildcard keyboard2_text_typed_password;
    touchgfx::TextAreaWithOneWildcard keyboard1_text_typed_hide_password;
    touchgfx::TextAreaWithOneWildcard keyboard2_text_typed_hide_password;
    touchgfx::ToggleButton keyboard1_pwd_eye_password;
    touchgfx::ToggleButton keyboard2_pwd_eye_password;
    touchgfx::BoxWithBorderButtonStyle< touchgfx::ClickButtonTrigger >  keyboard1_btn_show_password;
    touchgfx::BoxWithBorderButtonStyle< touchgfx::ClickButtonTrigger >  keyboard2_btn_show_password;
    touchgfx::TextButtonStyle< touchgfx::BoxWithBorderButtonStyle< touchgfx::ClickButtonTrigger >  >  keyboard_btn_enter_password;
    touchgfx::TextAreaWithOneWildcard text_info_1_password;
    touchgfx::TextArea text_info_2_password;
    touchgfx::TextArea placeholder_1_password;
    touchgfx::TextArea placeholder_2_password;
    touchgfx::Container s5_2_successPassword;
    touchgfx::WildcardTextButtonStyle< touchgfx::BoxWithBorderButtonStyle< touchgfx::ClickButtonTrigger >  >  btn_pwd_success;
    touchgfx::TextAreaWithOneWildcard text_pwd_success_info;
    touchgfx::Image image_pwd_success;
    touchgfx::Container s5_3_enrollFingerprint;
    touchgfx::Container fingerprint_images;
    touchgfx::Image fingerprint_1_image;
    touchgfx::Image fingerprint_2_image;
    touchgfx::Image fingerprint_3_image;
    touchgfx::Image fingerprint_success_image;
    touchgfx::Image fingerprint_error_image;
    touchgfx::Container fingerprint_right_hand_texts;
    touchgfx::TextArea fingerprint_right_hand_info1;
    touchgfx::TextArea fingerprint_right_hand_info2;
    touchgfx::Container fingerprint_left_hand_texts;
    touchgfx::TextArea fingerprint_left_hand_info1;
    touchgfx::TextArea fingerprint_left_hand_info2;
    touchgfx::Container fingerprint_common_texts;
    touchgfx::TextArea fingerprint_common_info1;
    touchgfx::TextAreaWithOneWildcard fingerprint_common_info2;
    touchgfx::TextAreaWithOneWildcard fingerprint_header;
    touchgfx::Container s5_4_successBiometric;
    touchgfx::WildcardTextButtonStyle< touchgfx::BoxWithBorderButtonStyle< touchgfx::ClickButtonTrigger >  >  btn_bio_success;
    touchgfx::TextAreaWithOneWildcard text_bio_success_info;
    touchgfx::Image image_bio_success;
    touchgfx::Container s6_alias;
    keyboard_encrypt_tagAlias keyboard_alias;
    touchgfx::IconButtonStyle< touchgfx::ClickButtonTrigger >  keyboard_btn_hide_alias;
    touchgfx::TextButtonStyle< touchgfx::BoxWithBorderButtonStyle< touchgfx::ClickButtonTrigger >  >  btn_assign_alias;
    textArea keyboard_text_area_alias;
    touchgfx::BoxWithBorderButtonStyle< touchgfx::ClickButtonTrigger >  keyboard_btn_show_alias;
    touchgfx::IconButtonStyle< touchgfx::ClickButtonTrigger >  keyboard_btn_enter_alias;
    touchgfx::TextAreaWithOneWildcard keyboard_text_typed_alias;
    touchgfx::TextArea keyboard_text_info_1_alias;
    touchgfx::TextArea keyboard_text_info_2_alias;
    touchgfx::Container s7_processing;
    touchgfx::TextArea processing_text;
    touchgfx::Container s8_success;
    touchgfx::Container success_msg_1;
    touchgfx::TextButtonStyle< touchgfx::BoxWithBorderButtonStyle< touchgfx::ClickButtonTrigger >  >  btn_success_1;
    touchgfx::TextArea success_1_text;
    touchgfx::Image success_1_image;
    touchgfx::Container success_msg_2;
    touchgfx::TextButtonStyle< touchgfx::BoxWithBorderButtonStyle< touchgfx::ClickButtonTrigger >  >  btn_success_2;
    touchgfx::TextArea success_2_text;
    touchgfx::Image success_2_image;
    close_btn close_button;

    /*
     * Wildcard Buffers
     */
    static const uint16_t TEXT_MULTI_NUM_SIZE = 5;
    touchgfx::Unicode::UnicodeChar text_multi_numBuffer[TEXT_MULTI_NUM_SIZE];
    static const uint16_t KEYBOARD1_TEXT_TYPED_PASSWORD_SIZE = 45;
    touchgfx::Unicode::UnicodeChar keyboard1_text_typed_passwordBuffer[KEYBOARD1_TEXT_TYPED_PASSWORD_SIZE];
    static const uint16_t KEYBOARD2_TEXT_TYPED_PASSWORD_SIZE = 45;
    touchgfx::Unicode::UnicodeChar keyboard2_text_typed_passwordBuffer[KEYBOARD2_TEXT_TYPED_PASSWORD_SIZE];
    static const uint16_t KEYBOARD1_TEXT_TYPED_HIDE_PASSWORD_SIZE = 45;
    touchgfx::Unicode::UnicodeChar keyboard1_text_typed_hide_passwordBuffer[KEYBOARD1_TEXT_TYPED_HIDE_PASSWORD_SIZE];
    static const uint16_t KEYBOARD2_TEXT_TYPED_HIDE_PASSWORD_SIZE = 45;
    touchgfx::Unicode::UnicodeChar keyboard2_text_typed_hide_passwordBuffer[KEYBOARD2_TEXT_TYPED_HIDE_PASSWORD_SIZE];
    static const uint16_t TEXT_INFO_1_PASSWORD_SIZE = 50;
    touchgfx::Unicode::UnicodeChar text_info_1_passwordBuffer[TEXT_INFO_1_PASSWORD_SIZE];
    static const uint16_t BTN_PWD_SUCCESS_SIZE = 25;
    touchgfx::Unicode::UnicodeChar btn_pwd_successBuffer[BTN_PWD_SUCCESS_SIZE];
    static const uint16_t TEXT_PWD_SUCCESS_INFO_SIZE = 5;
    touchgfx::Unicode::UnicodeChar text_pwd_success_infoBuffer[TEXT_PWD_SUCCESS_INFO_SIZE];
    static const uint16_t FINGERPRINT_COMMON_INFO2_SIZE = 5;
    touchgfx::Unicode::UnicodeChar fingerprint_common_info2Buffer[FINGERPRINT_COMMON_INFO2_SIZE];
    static const uint16_t FINGERPRINT_HEADER_SIZE = 5;
    touchgfx::Unicode::UnicodeChar fingerprint_headerBuffer[FINGERPRINT_HEADER_SIZE];
    static const uint16_t BTN_BIO_SUCCESS_SIZE = 25;
    touchgfx::Unicode::UnicodeChar btn_bio_successBuffer[BTN_BIO_SUCCESS_SIZE];
    static const uint16_t TEXT_BIO_SUCCESS_INFO_SIZE = 5;
    touchgfx::Unicode::UnicodeChar text_bio_success_infoBuffer[TEXT_BIO_SUCCESS_INFO_SIZE];
    static const uint16_t KEYBOARD_TEXT_TYPED_ALIAS_SIZE = 25;
    touchgfx::Unicode::UnicodeChar keyboard_text_typed_aliasBuffer[KEYBOARD_TEXT_TYPED_ALIAS_SIZE];

private:

    /*
     * Canvas Buffer Size
     */
    static const uint32_t CANVAS_BUFFER_SIZE = 4800;
    uint8_t canvasBuffer[CANVAS_BUFFER_SIZE];

    /*
     * Tick Counter Declarations
     */
    static const uint32_t TICK_TICKEVENT_INTERVAL = 10;
    uint32_t frameCountTickEventInterval;

    /*
     * Callback Declarations
     */
    touchgfx::Callback<screen_flow_encrypt_part_2ViewBase, const touchgfx::AbstractButtonContainer&> flexButtonCallback;
    touchgfx::Callback<screen_flow_encrypt_part_2ViewBase, const touchgfx::AbstractButton&> buttonCallback;

    /*
     * Callback Handler Declarations
     */
    void flexButtonCallbackHandler(const touchgfx::AbstractButtonContainer& src);
    void buttonCallbackHandler(const touchgfx::AbstractButton& src);

};

#endif // SCREEN_FLOW_ENCRYPT_PART_2VIEWBASE_HPP
